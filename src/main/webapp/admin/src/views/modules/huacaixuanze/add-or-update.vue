<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="花材选择" prop="huacaixuanze">
          <el-input v-model="ruleForm.huacaixuanze" 
              placeholder="花材选择" clearable  :readonly="ro.huacaixuanze"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `huacaixuanze/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huacaixuanzeCrossAddOrUpdateFlag = false;
                  this.parent.search();
